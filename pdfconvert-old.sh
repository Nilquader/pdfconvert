...
# Single pages in the middle of the magazine must be scanned first (currently sorts only correct, if there is only one single page)
...
while [ $sourcepage -lt $pdfpages ]; do
  ...
  if [ $height -lt $width ]; then # normale Doppelseiten
    ...
    echo "Source page $sourcepage is a single page and becomes $rightpagezeroed"
    cp "tempimages/rotated-$pagezeroed.ppm" "tempimages/cut-$rightpagezeroed.ppm"
  fi
